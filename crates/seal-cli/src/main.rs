...
fn main() -> Result<()> {
    let _telemetry = seal_cli::telemetry::init();
    let cli = Cli::parse();
    ...
    let (seal_root, workspace_root) = if let Some(path) = &cli.path {
        ...
        let seal_root = resolve_seal_root_from_path(path)
            ...
            .unwrap_or_else(|_| {
                seal_core::scm::git::detect_git_root(path).unwrap_or_else(|| path.clone())
            });
        ...
        let seal_root = resolve_seal_root_from_path(&workspace_root)
            ...
            .unwrap_or_else(|_| {
                seal_core::scm::git::detect_git_root(&workspace_root)
    ...
    match cli.command {
        ...
        Commands::Reviews(cmd) => match cmd {
            ...
            ReviewsCommands::List {
                ...
                let status_str = status.map(|s| match s {
                    seal_cli::cli::ReviewStatus::Open => "open",
                    seal_cli::cli::ReviewStatus::Approved => "approved",
                    seal_cli::cli::ReviewStatus::Merged => "merged",
                    seal_cli::cli::ReviewStatus::Abandoned => "abandoned",
                });
        ...
        Commands::Threads(cmd) => match cmd {
            ...
            ThreadsCommands::List {
                ...
                let status_str = status.map(|s| match s {
                    seal_cli::cli::ThreadStatus::Open => "open",
                    seal_cli::cli::ThreadStatus::Resolved => "resolved",
                });
                let since_dt = since
                    .map(|s| seal_cli::cli::commands::reviews::parse_since(&s))
                    .transpose()?;
                run_threads_list(
                    &seal_root,
                    &review_id,
        ...
        Commands::Ui => {
            let db_path = seal_root.join(".seal").join("index.db");
            let ctx = seal_core::core::CoreContext::new(&seal_root, &db_path)?;
            let services = ctx.services()?;
            seal_tui::run(&seal_root, services)?;
        }
